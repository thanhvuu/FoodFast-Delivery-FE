    ...
    const checkpoints = [
        {
            offset: 0,
            id: 'pickup',
            title: 'Nhận món tại bếp trung tâm',
            description: 'Đơn hàng đã được xác nhận và đang đóng gói.',
        },
        {
            offset: 2,
            id: 'takeoff',
            title: 'Drone cất cánh',
            description: 'Drone rời bãi đáp và tăng độ cao an toàn.',
        },
        {
            offset: 6,
            id: 'enroute',
            title: 'Đang giao hàng',
            description: 'Drone di chuyển đến khu vực giao với tốc độ ổn định.',
        },
        {
            offset: 9,
            id: 'arriving',
            title: 'Chuẩn bị hạ cánh',
            description: 'Drone giảm độ cao và thông báo cho khách chuẩn bị nhận hàng.',
        },
        {
            offset: 12,
            id: 'delivered',
            title: 'Hoàn tất giao hàng',
            description: `Đơn hàng sẽ được giao tại ${address}.`,
        },
    ];
...
const PlaceOrder = () => {
    ...
    const calculateDeliveryFee = (method) => {
        ...
        if (!option || cartTotal <= 0) return 0;
    ...
    const selectedDeliveryOption =
        DELIVERY_OPTIONS[deliveryMethod] ?? DELIVERY_OPTIONS.drone;
    ...
    // Lấy dữ liệu checkoutInfo nếu có
    useEffect(() => {
        const info = window.localStorage.getItem('checkoutInfo');
        if (info) {
            try {
                setForm(JSON.parse(info));
            } catch {
                // ignore
            }
        }
    ...
    const handleSubmit = async (e) => {
        ...
        // Gửi đơn hàng lên API json-server
        try {
            await createOrder({
